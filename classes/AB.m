classdef AB
    %AB collection of functions for empirical Bayes analysis of A/B tests.
    ...
    %This class is helpful for (1) calculating the posterior mean of quality as a
    %function of results of an experiment and a prior distribution of quality.
    %And for (2) performing maximum likelihood estimates of the distribution
    ...
    methods(Static = true)
        ...
        function L = L(z, sigma, beta, g)
            ...
            % Calculate integral
            L = AB.robust_integral(integrand, waypoints);
        end
        ...
        function L_cdf = L_cdf(z, sigma, beta, g)
            ...
            L_cdf = AB.robust_integral(integrand, waypoints);
        end
        ...
        function delta_bar = mean_posterior(z, sigma, beta, g)
            ...
            delta_bar = ...
                AB.robust_integral(integrand, waypoints) ./ ...
                AB.L(z, sigma, beta, g);
        end
        ...
        function z = mean_posterior_root(sigma, beta, g)
            ...
            if isscalar(sigma)
                ...
                f = @(z) AB.mean_posterior(z, sigma, beta, g);
...
%                 method.
                ...
                f = @(x) AB.mean_posterior_root(x, beta, g);
                z = arrayfun(f, sigma);
            end
        end
        ...
        function [beta_out, l, flag, output] = fit_g_no_gradient(z_data, sigma_data, beta_initial, g, max_iterations)
            ...
            %   g(delta, beta).
            obj = @(beta) -mean(AB.l(z_data, sigma_data, beta, g));
        ...
        function [beta_out, ...
            ...
            function [f, df, ddf, Omega] = obj(beta)
                ...
                LL      =  AB.L(z_data', sigma_data', beta, g);
                ...
                df1     =  AB.L(z_data', sigma_data', beta, dg);
                ...
                df2     =  AB.L(z_data', sigma_data', beta, dggaux);
        ...
        function [f, z_bar] = f(n, sigma_individual, beta, g, varargin)
            ...
            elseif isinf(n)
                ...
                f = AB.robust_integral(integrand, waypoints) - max(beta(1), 0);
                return;
            end
            ...
            % production function.
            if isempty(varargin)
%                 z_bar = AB.mean_posterior_root(sigma, beta, g);
                f_aux = @(z) -AB.f(n, sigma_individual, beta, g, z);
                z_guess = AB.mean_posterior_root(sigma, beta, g);
...
%                 ];
            ...
            f = AB.robust_integral(integrand, waypoints) - max(m, 0);
        end
    end
end
